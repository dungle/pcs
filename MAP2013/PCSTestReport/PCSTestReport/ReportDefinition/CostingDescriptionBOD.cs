...
namespace CostingDescriptionBOD
{
	[Serializable]
	public class CostingDescriptionBOD : MarshalByRefObject, IDynamicReport
	{
		public CostingDescriptionBOD()
		{
		}
		...
		public DataTable ExecuteReport(string pstrMonth, string pstrProductID, string pstrMakeItem, string pstrProductType)
		{
			...
			if (mLayoutFile == string.Empty)
				mLayoutFile = "CostingDescriptionBOD.xml";
			...
			ppvViewer.FormTitle = "Costing Description - BOD " + dtmDate.ToString("MMMM-yyyy");
		...
		private DataTable GetReportData(string pstrMonth, string pstrProductID, int pintMakeItem, string pstrProductType)
		{
			...
			try
			{
				string strSql = " DECLARE @PeriodID int, @FromDate datetime,@FromDateStr nvarchar(100),@ToDate datetime"
					...
					+ " E.Code CostElement, A.BeginQuantity,"
					+ " ISNULL(A.BeginCost,0) BeginCost,"
					+ " A.WOCompletionQty, A.ActualCost,"
					+ " CASE WHEN ISNULL(A.WOCompletionQty,0) <> 0 THEN"
					+ " 	((A.ActualCost*A.Quantity-A.BeginQuantity*A.BeginCost)/A.WOCompletionQty)"
					+ " ELSE 0"
					+ " END WOCompletionCost,"
					+ " (ISNULL(D.DSAmount,0)+ISNULL(OH_DSAmount,0)) DSAmount,ISNULL(D.RecycleAmount,0) + ISNULL(OH_RecycleAmount,0) RecycleAmount,ISNULL(D.AdjustAmount,0) + ISNULL(OH_AdjustAmount,0) AdjustAmount,"
					...
					+ " AND (sm.ShippedDate BETWEEN @FromDate AND @ToDate+1)),0) * A.ActualCost -"
					...
					+ " WHERE sd.ProductID = A.ProductID"
					+ " AND sm.PostDate BETWEEN @FromDate AND @ToDate+1),0) * A.ActualCost"
					+ " + ISNULL(D.DSAmount,0)"
					...
					+ " JOIN SO_ReturnedGoodsMaster sm ON sd.ReturnedGoodsMasterID = sm.ReturnedGoodsMasterID"
					+ " WHERE sd.ProductID = A.ProductID"
					+ " AND sm.PostDate BETWEEN @FromDate AND @ToDate+1),0) * A.ActualCost AS TotalCGS1,"
					+ " ITM_Category.Code AS Category"
					+ " FROM CST_ActualCostHistory A "
					+ " JOIN STD_CostElement E ON A.CostElementID = E.CostElementID"
				...
				strSql += " GROUP BY A.ActCostAllocationMasterID, ITM_Category.Code, P.Code, P.Description, P.Revision, P.MakeItem, M.Code, E.Code,A.BeginQuantity,"
					+ " A.BeginCost, A.WOCompletionQty, A.ActualCost,A.Quantity,D.DSAmount,D.OH_DSAmount,"
					+ " D.RecycleAmount,D.OH_RecycleAmount,D.AdjustAmount,D.OH_AdjustAmount,A.ProductID, A.CostElementID"
