...
namespace StdActCostComparision
{
	/// <summary>
	/// Compare Standard and Actual Cost
	/// </summary>
	[Serializable]
	public class StdActCostComparision : MarshalByRefObject, IDynamicReport
	{
		public StdActCostComparision()
		{
			//
...
//											decActualCost = 0;
		...
		private DataTable GetCostCenterRate(string pstrCCNID)
		{
			DataTable dtbData = new DataTable();
			...
			try 
			{
				string strSql = "SELECT * FROM STD_CostCenterRateDetail JOIN STD_CostCenterRateMaster"
		...
		private DataTable GetAllocation()
		{
			DataTable dtbData = new DataTable();
			...
			try 
			{
				string strSql = "SELECT * FROM CST_AllocationResult";
