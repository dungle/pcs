...
namespace SaleByCustomerBreakDown
{
	public class SaleByCustomerBreakDown : MarshalByRefObject, IDynamicReport
	{
		...
		public DataTable ExecuteReport(string pstrCCNID, string pstrFromDate, string pstrToDate, string pstrTypeCode, string pstrCustomerID, string pstrMakeItem)
		{
			...
			foreach (DataRow drowData in dtbReportData.Rows)
			{
				...
				if (drowActualCost.Length > 0)
				{
					try
					{
						...
						decRawCost = decRate * decCharge + decQuantity * decActualCost;
				...
				else
				{
					...
					if (drowStdCost.Length > 0)
					{
						try
						{
							...
							decRawCost = decRate * decCharge + decQuantity * decStdCost;
							drowReport[CostElementType.Material.ToString()] = decRawCost;
						}
						catch{}
					}
			...
			mLayoutFile = "SaleByCustomerBreakDown.xml";
			...
			ppvViewer.FormTitle = "Sale Amount and cost of goods sold break-down report (Classified By Customers)";
