...
namespace NIGURI2
{
	...
	/// </summary>
	[Serializable]
	public class NIGURI2 : MarshalByRefObject, IDynamicReport
	{
		#region Enum
			...
			string pstrCategoryID, string pstrPartyID, string pstrSourceID, string pstrModel, string pstrProductID)
		{
			...
			// begin data from MRP Result
			DataTable dtbBeginMRP = GetBeginQuantityOfNiguri(pstrLocationID);
			...
			DataTable dtbBeginMAP = new DataTable();
			if (dtmServerDate >= dtmFromDate)
			{
				dtbBeginData = GetBeginData(strItems, pstrLocationID, dtmFromDatePreMonth);
				dtbBeginMAP = GetBeginData(strItems, dtmFromDatePreMonth);
			}

			#region data for stock plan

			DataTable dtbPlanParent = GetDeliveryPlanForParent(strItems, dtmShiftFromDate);
			// get first valid work day of current month
			dtmFirstValidDay = GetFirtValidWorkday(arrOffDay, arrHolidays, dtmFromDate);
			// refine the delivery date
			foreach (DataRow drowData in dtbPlanParent.Rows)
			{
				// using StartTime of parent instead of WorkingDate
				DateTime dtmDate = (DateTime)drowData["StartTime"];
				// EndTime to check for over quantity from parent
			...
			DataTable dtbPlanSO = GetDeliveryPlanForSO(strItems, intMasterLocationID, dtmShiftFromDate);
			DataTable dtbPlanCPO = GetArrivalPlanFromCPO(strItems, dtmShiftFromDate);
			DataTable dtbPlanPO = GetArrivalPlanFromPO(strItems, intMasterLocationID, dtmShiftFromDate);
			...
			foreach (DataRow drowItem in dtbItems.Rows)
			{
				...
				bool blnHasArrivalPlan = false;
				bool blnHasArrivalActual = false;
				bool blnHasNG = false;
				bool blnHasAbnormal = false;
				...
				#region begin for stock plan from MRP

				decimal decBeginPlan = 0, decStockDeliveryPlan = 0, decStockArrivalPlan = 0;
				decimal decActualPlanDelivery = 0, decActualPlanArrival = 0;
				DateTime dtmAsOfDate = DateTime.MinValue;
				string strBeginPlanFilter = "ProductID = " + strProductID
					+ " AND AsOfDate <= '" + dtmFromDate.ToString("G") + "'";
				string strOrderBy = "AsOfDate DESC";
				DataRow[] drowsBeginPlan = dtbBeginMRP.Select(strBeginPlanFilter, strOrderBy);
				try
				{
					// MintNT (MAP) request: quantity for Master location instead of location
					decBeginPlan = Convert.ToDecimal(drowsBeginPlan[0]["QuantityMAP"]);
				}
				catch{}
				try
				{
					dtmAsOfDate = (DateTime)drowsBeginPlan[0]["AsOfDate"];
				}
				catch{}

				#endregion

				if (dtmAsOfDate != DateTime.MinValue)
				{
					DateTime dtmShiftAOD = dtmAsOfDate;
					GetStartAndEndTime(dtmAsOfDate, ref dtmShiftAOD, ref dtmTempDate, dtbWorkingTime);

					#region delivery plan for calculate stock plan

					try
					{
						decStockDeliveryPlan += Convert.ToDecimal(dtbPlanParent.Compute("SUM(Quantity)",
							"ComponentID = " + strProductID
							+ " AND WorkingDate >= '" + dtmAsOfDate.ToString("G") + "'"
							+ " AND WorkingDate < '" + dtmFromDate.ToString("G") + "'"));
					}
					catch{}
					try
					{
						decStockDeliveryPlan += Convert.ToDecimal(dtbPlanSO.Compute("SUM(Quantity)",
							"ProductID = " + strProductID
							+ " AND ScheduleDate >= '" + dtmShiftAOD.ToString("G") + "'"
					...
					#region arrival plan for calculate stock plan

					try
					{
						decStockArrivalPlan += Convert.ToDecimal(dtbPlanPO.Compute("SUM(Quantity)",
							"ProductID = " + strProductID
							+ " AND ScheduleDate >= '" + dtmShiftAOD.ToString("G") + "'"
							+ " AND ScheduleDate < '" + dtmShiftFromDate.ToString("G") + "'"));
					}
					...
					try
					{
						decStockArrivalPlan += Convert.ToDecimal(dtbPlanCPO.Compute("SUM(Quantity)",
							"ProductID = " + strProductID
							+ " AND EndTime >= '" + dtmShiftAOD.ToString("G") + "'"
				...
				// begin = current begin + in - out
				decBeginPlan = decBeginPlan + decStockArrivalPlan - decStockDeliveryPlan;

				if (dtmServerDate >= dtmFromDate) // past
				{
					#region OK
				...
				else
				{
					...
					decBeginQuantity = decBeginQuantity + decActualPlanArrival - decActualPlanDelivery;
				}
				...
				decimal decStockPlanOfPreDay = decBeginPlan;
				decimal decStockActualOKOfPreDay = decBeginQuantity;
				decimal decStockActualNGOfPreDay = decBeginQuantityNG;

				#region 07-05-2007, KienNT request

				//When viewing report for past month, we will retrieve data from balance and apply to stock plan
				if (dtmServerDate >= dtmFromDate) // past
				{
					try
					{
						decStockPlanOfPreDay = Convert.ToDecimal(dtbBeginMAP.Compute("SUM(Quantity)", strFilter));
					}
					catch{}
				}
				...
				for (int i = 1; i <= intDaysInMonth; i++)
				{
					...
					try
					{
						// change request: Arrival plan retrieve from approved PO (make and none make)
					...
					if (dtmDay > new DateTime(dtmServerDate.Year, dtmServerDate.Month, dtmServerDate.Day))
						decStockActualOKOfPreDay = decStockActualOKOfPreDay - decTotalDeliveryPlan + decArrivalPlan;
		...
		private DataTable GetDeliveryPlanForParent(string strItems, DateTime pdtmToDate)
		{
			OleDbConnection oconPCS = null;
			OleDbCommand cmdPCS = null;
			OleDbDataAdapter odadPCS = null;
			try
			{
				...
				string strSql = "SELECT ISNULL((PRO_DCPResultDetail.Quantity * ITM_BOM.Quantity)"
					...
					+ " JOIN ITM_BOM ON PRO_DCPResultMaster.ProductID = ITM_BOM.ProductID"
					+ " WHERE ITM_BOM.ComponentID IN (" + strItems + ")"
					+ " AND PRO_DCPResultDetail.StartTime < ?"
					+ " ORDER BY PRO_DCPResultMaster.ProductID, ITM_BOM.ComponentID, StartTime";
		...
		private DataTable GetDeliveryPlanForSO(string strItems, int pintMasterLocationID, DateTime pdtmToDate)
		{
			OleDbConnection oconPCS = null;
			OleDbCommand ocmdPCS = null;
			OleDbDataAdapter odadPCS = null;
		...
		private DataTable GetArrivalPlanFromPO(string strItems, int pintMasterLocationID, DateTime pdtmToDate)
		{
			OleDbConnection oconPCS = null;
			OleDbCommand ocmdPCS = null;
			OleDbDataAdapter odadPCS = null;
		...
		private DataTable GetArrivalPlanFromCPO(string strItems, DateTime pdtmToDate)
		{
			OleDbConnection oconPCS = null;
			OleDbCommand ocmdPCS = null;
			OleDbDataAdapter odadPCS = null;
		...
		private DataTable GetBeginQuantityOfNiguri(string pstrLocationID)
		{
			OleDbConnection oconPCS =null;
			OleDbCommand ocmdPCS = null;
			try 
			{
				string strSql=	"SELECT * FROM IV_BeginMRP WHERE LocationID = " + pstrLocationID
		...
		private DataTable GetBeginData(string pstrItems, DateTime pdtmMonth)
		{
			OleDbConnection oconPCS = null;
			try
			{
				oconPCS = new OleDbConnection(mConnectionString);
				string strSql=	"SELECT BB.ProductID, SUM(ISNULL(OHQuantity,0)) AS Quantity"
					+ " FROM IV_BalanceBin BB JOIN MST_Bin B ON BB.BinID = B.BinID"
					+ " WHERE DATEPART(year, EffectDate) = " + pdtmMonth.Year
					+ " AND DATEPART(month, EffectDate) = " + pdtmMonth.Month
					+ " AND BinTypeID IN (" + (int)BinTypeEnum.BF + "," + (int)BinTypeEnum.OK + ")";
				if (pstrItems != null && pstrItems != string.Empty)
					strSql += " AND BB.ProductID IN (" + pstrItems + ")";
				strSql += " GROUP BY BB.LocationID, BB.ProductID, B.BinTypeID"
					+ " ORDER BY BB.LocationID, BB.ProductID, B.BinTypeID";
