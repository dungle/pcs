...
namespace PCSUtils.Framework.TableFrame
{
	...
	public class ViewSingleRecord : Form
	{
		...
		private const string THIS = "PCSUtils.Framework.TableFrame.ViewSingleRecord";
		...
		public ViewSingleRecord(string pstrTableName)
		{
			//
...
this.Cursor = Cursors.Default;
				...
				//HACK: DuongNA 2005-10-12
				//To avoid the case focus in combobox cause user cannot escape by ESC key
				btnClose.Select();
				//btnClose.Focus();
				//End DuongNA 2005-10-12
			}
		...
		private void btnAdd_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				foreach (Control objControl in this.pnlControls.Controls)
				{
				...
				this.btnDelete.Enabled = false;
				btnEdit.Enabled = false;
				// focus to first editable control
				if (pnlControls.Controls.Count > 0)
				{
					pnlControls.Controls[1].Focus();
				}
			}
			catch (PCSException ex)
			{
		...
		private void LoadControls(ArrayList parrFields, DataRow pdrowData)
		{
			...
			try
			{
				...
				for (int i = 0; i < parrFields.Count; i++)
				{
					...
					if (voTableField.Width <= 1)
						continue;
					if (this.pnlControls.Controls.Count > 0)
					{
						objLastControl = this.pnlControls.Controls[this.pnlControls.Controls.Count -1];
					}
					DataColumn dcolData = pdrowData.Table.Columns[voTableField.FieldName];
					// do not draw control with column width = 1
					...
					this.pnlControls.Controls.Add(lblFieldName);
					#endregion
					...
					if ((dcolData.DataType != typeof(bool)) && (dcolData.DataType != typeof(DateTime)))
					{
							...
							|| ((voTableField.FromTable != null) && (voTableField.FromTable.Trim() != string.Empty)))
						{
							...
							this.pnlControls.Controls.Add(cboField);
							if ((voTableField.Items != null) && (voTableField.Items.Trim() != string.Empty))
							{
					...
					#region textbox
						...
						else // textbox
						{
							...
							txtField.DisableOnNoData = false;
							...
							if (dcolData.DataType == typeof(string))
							...
							this.pnlControls.Controls.Add(txtField);
							if (this.mFormAction == EnumAction.Default)
							{
					...
					if (dcolData.DataType == typeof(bool))
					{
						...
						this.pnlControls.Controls.Add(objFieldControl);
					...
					{
						C1DateEdit dtmDate = new C1DateEdit();
						dtmDate.Name = DATEPICKER_PREFIX + dcolData.ColumnName;
						dtmDate.FormatType = FormatTypeEnum.CustomFormat;
						dtmDate.CustomFormat = Constants.DATETIME_FORMAT;
						dtmDate.TextAlign = HorizontalAlignment.Center;
						dtmDate.VisibleButtons = DropDownControlButtonFlags.DropDown;
						try
						{
							dtmDate.Value = DateTime.Parse(pdrowData[dcolData].ToString());
						}
						catch
						{
							dtmDate.Value = DateTime.Now;
						}
						dtmDate.Location = new Point(CONTROL_X_LOCATION, lblFieldName.Location.Y);
						dtmDate.Size = new Size(voTableField.Width, DEFAULT_HEIGHT);
						dtmDate.TabIndex = ++ intTabIndex;
						dtmDate.Tag = voTableField;
						dtmDate.ValueIsDbNull = true;
						dtmDate.DisableOnNoData = false;
						this.pnlControls.Controls.Add(dtmDate);
