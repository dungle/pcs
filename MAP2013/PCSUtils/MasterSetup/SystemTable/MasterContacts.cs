...
{
	/// <summary>
	/// Summary description for MasterContacts.
	/// </summary>
	public class MasterContacts : Form
	{
		...
		private const string THIS = "PCSUtils.MasterSetup.SystemTable.MasterContacts";
		private MasterContactBO boMasterContact = new MasterContactBO();
		private UtilsBO boUtils = new UtilsBO();
		private MST_PartyVO voParty = new MST_PartyVO();
		private MST_PartyContactVO voPartyContact = new MST_PartyContactVO();
		...
		public MasterContacts(MST_PartyVO pvoParty)
		{
			//
			// Required for Windows Form Designer support
			//
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(MasterContacts));
			...
			this.Name = "MasterContacts";
			this.RightToLeft = ((System.Windows.Forms.RightToLeft)(resources.GetObject("$this.RightToLeft")));
			this.StartPosition = ((System.Windows.Forms.FormStartPosition)(resources.GetObject("$this.StartPosition")));
			this.Text = resources.GetString("$this.Text");
			this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.MasterContacts_KeyDown);
			this.Closing += new System.ComponentModel.CancelEventHandler(this.MasterContacts_Closing);
			this.Load += new System.EventHandler(this.MasterContacts_Load);
			...
			try
			{
				switch (mFormMode)
				{
					case EnumAction.Default:
						...
						if (voPartyContact.PartyContactID > 0)
						{
		...
		private bool SaveData()
		{
			try
			{
				voPartyContact.PartyID = voParty.PartyID;
				voPartyContact.Code = txtContactCode.Text.Trim();
				voPartyContact.Name = txtName.Text.Trim();
				voPartyContact.Title = txtTitle.Text.Trim();
				voPartyContact.Phone = txtPhone.Text.Trim();
				voPartyContact.Ext = txtExtension.Text.Trim();
				voPartyContact.Fax = txtFax.Text.Trim();
				voPartyContact.Email = txtEmail.Text.Trim();
				voPartyContact.Memo = txtMemo.Text.Trim();
				switch (mFormMode)
				{
					case EnumAction.Add:
						voPartyContact.PartyContactID = boMasterContact.AddReturnID(voPartyContact);
						break;
					case EnumAction.Edit:
						boMasterContact.Update(voPartyContact);
						break;
				}
				return true;
			}
		...
		private void FillData(DataRow pdrowData)
		{
			try
			{
				...
				voPartyContact = (MST_PartyContactVO)boMasterContact.GetObjectVO(int.Parse(pdrowData[MST_PartyContactTable.PARTYCONTACTID_FLD].ToString()), string.Empty);				
				// load data to form
				LoadDataToControl(voPartyContact);
				// switching form mode
				SwitchFormMode();
			}
			catch (PCSException ex)
		...
		/// 0: other else
		private bool SelectContactCode(string pstrMethodName, bool pblnOpenOnly)
		{
			try				
			{	
				Hashtable htCondition = new Hashtable();				
				htCondition.Add(MST_PartyContactTable.PARTYID_FLD, voParty.PartyID);				
				
				//Call OpenSearchForm for selecting TransNo	
				DataRowView drwResult = FormControlComponents.OpenSearchForm(MST_PartyContactTable.TABLE_NAME, MST_PartyContactTable.CODE_FLD , txtContactCode.Text, htCondition, pblnOpenOnly);
		...
		private void txtContactCode_Validating(object sender, CancelEventArgs e)
		{
			...
			try
			{
				...
				{					
					voPartyContact = new MST_PartyContactVO();
					voPartyContact.PartyContactID = 0;
					LoadDataToControl(voPartyContact);
					SwitchFormMode();
					return;
				}
		...
		///    </Notes>
		//**************************************************************************
		private void MasterContacts_Load(object sender, EventArgs e)
		{
			...
			const string METHOD_NAME = THIS + ".MasterContacts_Load()";
			try
			{
				this.Name = THIS;
				//Set form security
...
this.Cursor = Cursors.Default;
				...
				{
					// do delete action
					boMasterContact.Delete(voPartyContact);
		...
		///    </Notes>
		//**************************************************************************
		private void MasterContacts_Closing(object sender, CancelEventArgs e)
		{
		...
		private void MasterContacts_KeyDown(object sender, KeyEventArgs e)
		{
			const string METHOD_NAME = THIS + ".MasterContacts_KeyDown()";
