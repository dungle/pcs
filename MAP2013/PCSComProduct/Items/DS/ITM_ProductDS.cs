	...
	public class ITM_ProductDS 
	{
		...
		public object GetProductInfo(int pintID)
		{
			...
			try
			{
				string strSql = "SELECT "
                    ...
                    + ITM_ProductTable.ITEMGROUPID_FLD + ", "
                    + ITM_ProductTable.PRODUCTCLASSIFIEDID_FLD + ", "
					+ ITM_ProductTable.ORDERPOINT_FLD
					+ " FROM " + ITM_ProductTable.TABLE_NAME
				...
				while (odrPCS.Read())
				{
					...
					objObject.Description = odrPCS[ITM_ProductTable.DESCRIPTION_FLD].ToString().Trim();
					objObject.SetupDate = DateTime.Parse(odrPCS[ITM_ProductTable.SETUPDATE_FLD].ToString().Trim());
					objObject.VAT = odrPCS[ITM_ProductTable.VAT_FLD] != DBNull.Value ? float.Parse(odrPCS[ITM_ProductTable.VAT_FLD].ToString().Trim()) : NUMBER_EMPTY_VALUE;
					objObject.ImportTax = odrPCS[ITM_ProductTable.IMPORTTAX_FLD] != DBNull.Value ? float.Parse(odrPCS[ITM_ProductTable.IMPORTTAX_FLD].ToString().Trim()) : NUMBER_EMPTY_VALUE;
					objObject.ExportTax = odrPCS[ITM_ProductTable.EXPORTTAX_FLD] != DBNull.Value ? float.Parse(odrPCS[ITM_ProductTable.EXPORTTAX_FLD].ToString().Trim()) : NUMBER_EMPTY_VALUE;
					objObject.SpecialTax = odrPCS[ITM_ProductTable.SPECIALTAX_FLD] != DBNull.Value ? float.Parse(odrPCS[ITM_ProductTable.SPECIALTAX_FLD].ToString().Trim()) : NUMBER_EMPTY_VALUE;
					...
					objObject.Length = odrPCS[ITM_ProductTable.LENGTH_FLD] != DBNull.Value ? Decimal.Parse(odrPCS[ITM_ProductTable.LENGTH_FLD].ToString().Trim()) : NUMBER_EMPTY_VALUE;
					objObject.Width = odrPCS[ITM_ProductTable.WIDTH_FLD] != DBNull.Value ? Decimal.Parse(odrPCS[ITM_ProductTable.WIDTH_FLD].ToString().Trim()) : NUMBER_EMPTY_VALUE;
					objObject.Height = odrPCS[ITM_ProductTable.HEIGHT_FLD] != DBNull.Value ? Decimal.Parse(odrPCS[ITM_ProductTable.HEIGHT_FLD].ToString().Trim()) : NUMBER_EMPTY_VALUE;
					objObject.Weight = odrPCS[ITM_ProductTable.WEIGHT_FLD] != DBNull.Value ? Decimal.Parse(odrPCS[ITM_ProductTable.WEIGHT_FLD].ToString().Trim()) : NUMBER_EMPTY_VALUE;
					if (odrPCS[ITM_ProductTable.FINISHEDGOODS_FLD] != DBNull.Value)
					{
						objObject.FinishedGoods = bool.Parse(odrPCS[ITM_ProductTable.FINISHEDGOODS_FLD].ToString().Trim());
					}
                    ...
                    if (odrPCS[ITM_ProductTable.ITEMGROUPID_FLD] != DBNull.Value)
                    {
                        int itemGroupId;
                        int.TryParse(odrPCS[ITM_ProductTable.ITEMGROUPID_FLD].ToString(), out itemGroupId);
                        objObject.ItemGroupID = itemGroupId > 0 ? itemGroupId : NUMBER_EMPTY_VALUE;
                    }
                    if (odrPCS[ITM_ProductTable.PRODUCTCLASSIFIEDID_FLD] != DBNull.Value)
                    {
                        int classifiedId;
                        int.TryParse(odrPCS[ITM_ProductTable.PRODUCTCLASSIFIEDID_FLD].ToString(), out classifiedId);
                        objObject.ProductClassifiedID = classifiedId > 0 ? classifiedId : NUMBER_EMPTY_VALUE;
                    }
		...
		public void UpdateProductInfo(object pobjObjecVO)
		{
			...
			try
			{
                ...
                ocmdPCS.Parameters.Add(new OleDbParameter(ITM_ProductTable.ITEMGROUPID_FLD, OleDbType.Integer));
                ocmdPCS.Parameters[ITM_ProductTable.ITEMGROUPID_FLD].Value = objObject.ItemGroupID > 0
                    ? (object)objObject.ItemGroupID
                    : DBNull.Value;

                ocmdPCS.Parameters.Add(new OleDbParameter(ITM_ProductTable.PRODUCTCLASSIFIEDID_FLD, OleDbType.Integer));
                ocmdPCS.Parameters[ITM_ProductTable.PRODUCTCLASSIFIEDID_FLD].Value = objObject.ProductClassifiedID > 0
                    ? (object)objObject.ProductClassifiedID
                    : DBNull.Value;
	    ...
	    {
            const string methodName = THIS + ".GetItemGroupAndClassified()";
            ...
            {
                var strSql = "SELECT "
                    + ITM_ItemGroupTable.ITEMGROUPID_FLD + ","
                    + ITM_ItemGroupTable.CODE_FLD + ","
                    + ITM_ItemGroupTable.DESCRIPTION_FLD
                    + " FROM " + ITM_ItemGroupTable.TABLE_NAME + ";";
                strSql += string.Format("SELECT {0}, {1}, {2} FROM {3}",
                    ITM_ProductClassifiedTable.PRODUCTCLASSIFIEDID_FLD, ITM_ProductClassifiedTable.CODE_FLD,
                    ITM_ProductClassifiedTable.DESCRIPTION_FLD, ITM_ProductClassifiedTable.TABLE_NAME);
                dbConnection = new OleDbConnection(Utils.Instance.OleDbConnectionString);
                ...
                resultDataSet.Tables[0].TableName = ITM_ItemGroupTable.TABLE_NAME;
                resultDataSet.Tables[1].TableName = ITM_ProductClassifiedTable.TABLE_NAME;
